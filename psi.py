...
def calculate_psi_no_pandas(
    ...
    """
    Calcula PSI para múltiplas features sem usar Pandas, determinando automaticamente os meses disponíveis.
    ...
    schema = "feature string, analysis_month string, baseline_window string, psi double, samples_current long, samples_baseline long"
    ...
    for feature in features:
        ...
        # 3. Filtrar dados não-nulos para a feature atual
        ...
        # 5. Calcular bins usando os primeiros N meses como baseline
        ...
        for i in range(baseline_months, len(months)):
            ...
            results = results.unionByName(psi_row.select("feature", "analysis_month", "baseline_window", 
                                                       "psi", "samples_current", "samples_baseline", "psi_category"))
...
# 1. Configuração inicial do Spark
from pyspark.sql import SparkSession
spark = SparkSession.builder \
    .appName("PSI Calculation No Pandas") \
    .config("spark.sql.shuffle.partitions", "200") \
    .getOrCreate()

# 2. Carregar dados
df = spark.read.parquet("caminho/para/seu/dados")

# 3. Definir features para análise
features = ["score_credito", "valor_transacao", "idade_cliente", "qtde_produtos"]

# 4. Calcular PSI
psi_results = calculate_psi_no_pandas(
    df=df,
    date_col="dt_ref",
    features=features,
    num_bins=10,
...
psi_results.show(truncate=False)

psi_results.write.mode("overwrite").parquet("caminho/para/resultados_psi")
